...
const agregarMedicamentoSchema = z.object({
  medicamentoId: z.string().uuid('ID de medicamento inválido').optional(),

  medicamentoPersonalizado: z.string().min(2, 'El nombre del medicamento debe tener al menos 2 caracteres').optional(),
  ...
  (data) => data.medicamentoId || data.medicamentoPersonalizado,
  {
  ...
  (data) => !(data.medicamentoId && data.medicamentoPersonalizado),
  {
    message: 'No puede proporcionar medicamentoId y medicamentoPersonalizado al mismo tiempo',
  }
);
